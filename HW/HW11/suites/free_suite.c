...
START_TEST(test_free_empty_freelist_lists) {
    ...
    ck_assert_ptr_eq(address_list, meta);
    ck_assert(!address_list -> next_addr);
    ck_assert(!address_list -> prev_addr);

    //check size_list
    ck_assert(size_list);
    ck_assert_ptr_eq(size_list, meta);
...
START_TEST(test_free_empty_freelist_insert) {
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
    ck_assert_ptr_eq(meta, my_sbrk_fake_heap + 64);
...
START_TEST(test_free_no_merge1_lists) {
    ...
    ck_assert_ptr_eq(address_list, A);
    ck_assert_ptr_eq(A -> next_addr, B);
    ck_assert_ptr_eq(B -> next_addr, meta);
    ck_assert_ptr_eq(meta -> next_addr, C);
    ck_assert(!C -> next_addr);
    //check address list back to front
    ck_assert_ptr_eq(C -> prev_addr, meta);
    ck_assert_ptr_eq(meta -> prev_addr, B);
    ck_assert_ptr_eq(B -> prev_addr, A);
    ck_assert(!A -> prev_addr);

    ck_assert(size_list);
    //check size list front to back
    ck_assert_ptr_eq(meta, size_list);
    ck_assert_ptr_eq(meta -> next_size, B);
    ck_assert_ptr_eq(B -> next_size, A);
    ck_assert_ptr_eq(A -> next_size, C);
    ck_assert(!C -> next_size);
    //check size list back to front
    ck_assert_ptr_eq(C -> prev_size, A);
    ck_assert_ptr_eq(A -> prev_size, B);
    ck_assert_ptr_eq(B -> prev_size, meta);
...
START_TEST(test_free_no_merge1_insert) {
    ...
    ck_assert_ptr_eq(meta, (uint8_t*)B + B -> size + 8);
}
END_TEST
...
START_TEST(test_free_no_merge2_lists) {
    ...
    //check address list front to back
    ck_assert_ptr_eq(address_list, meta);
    ck_assert_ptr_eq(meta -> next_addr, A);
    ck_assert_ptr_eq(A -> next_addr, B);
    ck_assert_ptr_eq(B -> next_addr, C);
    ck_assert(!C -> next_addr);
    //check address list back to front
    ck_assert_ptr_eq(C -> prev_addr, B);
    ck_assert_ptr_eq(B -> prev_addr, A);
    ck_assert_ptr_eq(A -> prev_addr, meta);
    ck_assert(!meta -> prev_addr);

    ck_assert(size_list);
    //check size list front to back
    ck_assert_ptr_eq(B, size_list);
    ck_assert_ptr_eq(B -> next_size, A);
    ck_assert_ptr_eq(A -> next_size, meta);
    ck_assert_ptr_eq(meta -> next_size, C);
    ck_assert(!C -> next_size);
    //check size list back to front
    ck_assert_ptr_eq(C -> prev_size, meta);
    ck_assert_ptr_eq(meta -> prev_size, A);
    ck_assert_ptr_eq(A -> prev_size, B);
...
START_TEST(test_free_no_merge2_insert) {
    ...
    ck_assert_ptr_eq(meta, my_sbrk_fake_heap);
}
END_TEST
...
START_TEST(test_free_left_merge2_lists) {
    ...
    ck_assert(size_list);
    //check size list front to back
    ck_assert_ptr_eq(A, size_list);
    ck_assert_ptr_eq(A -> next_size, B);
    ck_assert(!B -> next_size);
    //check size list back to front
    ck_assert_ptr_eq(B -> prev_size, A);
...
START_TEST(test_free_right_merge1_lists) {
    ...
    //check address list front to back
    ck_assert_ptr_eq(address_list, meta);
    ck_assert_ptr_eq(meta -> next_addr, B);
    ck_assert_ptr_eq(B -> next_addr, C);
    ck_assert(!C -> next_addr);
    //check address list back to front
    ck_assert_ptr_eq(C -> prev_addr, B);
    ck_assert_ptr_eq(B -> prev_addr, meta);
    ck_assert(!meta -> prev_addr);

    ck_assert(size_list);
    //check size list front to back
    ck_assert_ptr_eq(B, size_list);
    ck_assert_ptr_eq(B -> next_size, C);
    ck_assert_ptr_eq(C -> next_size, meta);
    ck_assert(!meta -> next_size);
    //check size list back to front
    ck_assert_ptr_eq(meta -> prev_size, C);
    ck_assert_ptr_eq(C -> prev_size, B);
    ck_assert(!B -> prev_size);
}
END_TEST
...
START_TEST(test_free_right_merge2_insert) {
    ...
    ck_assert_ptr_eq(meta, (uint8_t*)C - 255);
}
END_TEST
...
START_TEST(test_free_double_merge1_lists) {
    ...
    //check address list front to back
    ck_assert_ptr_eq(address_list, A);
    ck_assert_ptr_eq(A -> next_addr, C);
    ck_assert(!C -> next_addr);
    //check address list back to front
    ck_assert_ptr_eq(C -> prev_addr, A);
    ck_assert(!A -> prev_addr);
...
START_TEST(test_free_double_decker_lists) {
    ...
    ck_assert(address_list);
    //check address list 
    ck_assert_ptr_eq(A, address_list);
